...
//            sys_write(STDOUT,"No fue posible leer lo ingresado, por favor intente nuevamente",MAX_BUFFER_SIZE);
//        }else{
//            analyze_buffer();
//        }
//    }
...
//    for(; !pipe && !end  && !background; i++){                           // Recorrera siempre y cuando quede espacio para los argumentos o hasta llegar a un pipe o \0
...
//        executable_t exec_a = {get_program_name(program_a),program_a,i,aux_a,!background};
//        sys_exec(&exec_a);
//        return;
//    }
...
//    for(; i < MAX_ARGS_SIZE && !end && !background; i++){      // Recorrera siempre y cuando quede espacio para los argumentos o hasta llegar a un pipe o \0
//        new_token = str_tok(buffer+prev_token+1, ' ');          // Obtenemos el proximo token, el cual puede ser un nuevo argumento, | o \0
//        prev_token++;
...
//    //TODO: agregar pipe
//    executable_t exec_a = {get_program_name(program_a),program_a,arg_c_a,aux_a,!background};
//    executable_t exec_b = {get_program_name(program_b),program_b,i,aux_a,!background};
//
//
//
//
